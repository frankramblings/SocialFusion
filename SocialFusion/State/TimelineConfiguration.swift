...
class TimelineConfiguration {
    ...
    private init() {
        guard let path = Bundle.main.path(forResource: "Info", ofType: "plist"),
              ...
              let timelineConfig = plist["SocialFusionTimelineConfiguration"] as? [String: Any] else {
    ...
    var positionPersistenceEnabled: Bool {
        guard let positionConfig = config["PositionPersistence"] as? [String: Any] else { return true }
    ...
    var maxHistorySize: Int {
        guard let positionConfig = config["PositionPersistence"] as? [String: Any] else { return 10 }
        return positionConfig["MaxHistorySize"] as? Int ?? 10
    }
    
    var autoSaveInterval: TimeInterval {
        guard let positionConfig = config["PositionPersistence"] as? [String: Any] else { return 5.0 }
    ...
    var fallbackStrategy: FallbackStrategy {
        guard let positionConfig = config["PositionPersistence"] as? [String: Any],
              let strategyString = positionConfig["FallbackStrategy"] as? String else { 
    ...
    var timelineLogging: Bool {
        guard let debugConfig = config["Debug"] as? [String: Any] else { return false }
        return debugConfig["TimelineLogging"] as? Bool ?? false
    }
    
    var positionLogging: Bool {
        guard let debugConfig = config["Debug"] as? [String: Any] else { return false }
        return debugConfig["PositionLogging"] as? Bool ?? false
    }
    
    var performanceLogging: Bool {
        guard let debugConfig = config["Debug"] as? [String: Any] else { return false }
        return debugConfig["PerformanceLogging"] as? Bool ?? false
    }
    
    var verboseMode: Bool {
        guard let debugConfig = config["Debug"] as? [String: Any] else { return false }
        return debugConfig["VerboseMode"] as? Bool ?? false
    }
