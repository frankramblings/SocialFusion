...
class SmartPositionManager: ObservableObject {
    ...
    init() {
        ...
        if config.verboseMode {
            ...
            print("  iCloud Sync: \(config.iCloudSyncEnabled)")
    ...
    func restorePosition<T: Identifiable>(
        ...
        if config.positionLogging {
            print("🎯 Smart restoration for \(entries.count) entries, target: \(targetPostId ?? "none"), strategy: \(strategy)")
        }
    ...
    private func findSmartPosition<T: Identifiable>(
        ...
        guard let targetPostId = targetPostId,
              let lastSnapshot = findRelevantSnapshot(for: targetPostId) else {
            return nil
        }
        ...
        if let temporalMatch = findTemporalMatch(in: entries, around: targetTime, window: timeWindow) {
    ...
    private func applyFallbackStrategy<T: Identifiable>(
        ...
        if config.positionLogging {
            print("📋 Applied fallback strategy '\(strategy)': index=\(result.index?.description ?? "nil"), offset=\(result.offset)")
        }
    ...
    private func recordPositionSnapshot(_ snapshot: PositionSnapshot) {
        ...
        if config.positionLogging {
            print("📝 Recorded position snapshot: \(snapshot.postId) (\(snapshot.restorationMethod))")
        }
    }
    ...
    private func setupAutoSync() {
        ...
        iCloudSyncTimer = Timer.scheduledTimer(withTimeInterval: config.autoSaveInterval, repeats: true) { [weak self] _ in
            Task {
    ...
    private func loadPositionHistory() {
        guard let data = userDefaults.data(forKey: "SmartPositionHistory"),
              let history = try? JSONDecoder().decode([PositionSnapshot].self, from: data) else {
            return
        }
    ...
    func getRestorationSuggestions<T: Identifiable>(for entries: [T]) -> [RestorationSuggestion] where T.ID == String {
        ...
        for snapshot in recentSnapshots {
            if let index = entries.firstIndex(where: { $0.id == snapshot.postId }) {
                suggestions.append(RestorationSuggestion(
                    title: "Continue where you left off",
                    description: "Post from \(snapshot.timestamp.formatted(.relative(presentation: .named)))",
...
struct PositionSnapshot: Codable {
    ...
    static func from(cloudKitRecord record: CKRecord) -> PositionSnapshot? {
        guard let postId = record["postId"] as? String,
              ...
              let method = RestorationMethod(rawValue: methodString) else {
            return nil
        }
