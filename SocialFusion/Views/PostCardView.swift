    ...
    private func displayLinksAndMedia(for post: Post) -> some View {
        VStack(spacing: 12) {
            ...
            if let links = extractLinks(from: post.content), !links.isEmpty {
                // Limit to max 1 link preview to avoid excessive network requests
                ForEach(Array(links.prefix(1).enumerated()), id: \.element.absoluteString) {
                    index, url in
                    LinkPreview(url: url)
...
// Zoomable image view with pinch and double-tap to zoom
